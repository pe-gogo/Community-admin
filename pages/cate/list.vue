	...
	export default {
		data() {
			...
			return {
				...
				table1:{
					// 表格数据请求地址
					action:"admin/cate/kh/cate.getList",
					// 表格字段显示规则
					columns:[
						{ key:"id", title:"id", type:"id", width:80, sortable:"custom" },
						{ key:"name", title:"类别名", type:"text", width:160, sortable:"custom"  },
						{ key:"img", title:"图片", type:"text", width:220 },
					],
					// 多选框选中的值
					multipleSelection:[],
					// 当前高亮的记录
				...
				queryForm1:{
					...
					columns:[
						{ key:"_id", title:"备注", type:"text", width:160 },
						{ key:"name", title:"备注", type:"text", width:160 },
					]
				},
				form1:{
					// 表单请求数据，此处可以设置默认值
					...
					props: {
						...
						// 表单字段显示规则
						columns:[
							{ key:"name", title:"类别名称", type:"text", placeholder:"请输入类别名称" },
							{ key:"img", title:"图片地址", type:"text", placeholder:"请输入图片地址" }
						],
						// 表单验证规则
						rules:{
		...
		methods: {
			...
			// 显示修改页面
			updateBtn({ item }){
				that.form1.props.action = 'admin/cate/kh/cate.update';
